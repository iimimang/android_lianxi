...
public class CalendarUtil {
     ...
     * 用于保存中文的月份
     */
    public final static String CHINESE_NUMBER[] = {"一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "腊"};
    ...
    public String toString(int mLuchYear,int mLuchMonth,int mLuchDay) {
        String message = "";
        int n = mLuchDay % 10 == 0 ? 9 : mLuchDay % 10 - 1;
        message = getChinaCalendarMsg(mLuchYear, mLuchMonth, mLuchDay);
        if (StringUtil.isNullOrEmpty(message)) {
    ...
    public String getDay() {
            ...
            return mLuchYear + "年" + (isLoap ? "闰" : "") + CHINESE_NUMBER[mLuchMonth - 1] + "月"
                    + getChinaDayString(mLuchDay);
    }
    public String getDay(int mLuchYear, int mLuchMonth, int mLuchDay) {
    ...
    public String getmon() {
		...
		try {
			cal.setTime(Lunar.chineseDateFormat.parse(CalendarView.calStartDate.get(Calendar.YEAR)+"年"+NumberHelper.LeftPad_Tow_Zero(CalendarView.calStartDate
					.get(Calendar.MONTH) + 1)+"月"+CalendarView.calStartDate
		...
		String str2 = lunar.toString3();  
    	
        return str2;
    }
    
    public String getmon1(int day) {
    	int num = CalendarView.calStartDate.get(Calendar.MONTH) - 1;
    	if(num == 12){
    		num = 1;
    		CalendarView.calStartDate.set(Calendar.DAY_OF_MONTH, 1);
    	}
    	if(num == -1){
    		num = 11;
    		CalendarView.calStartDate.set(Calendar.DAY_OF_MONTH, 1);
        ...
        return CHINESE_NUMBER[num] + "月"+ getChinaDayString(day)+ "日";
    }
}
