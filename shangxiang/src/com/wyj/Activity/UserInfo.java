...
public class UserInfo extends Activity implements OnClickListener {
	...
	private void findViewById() {
		...
		if(MyApplication.getInstances().getSex()==1){
			...
			ColorStateList csl = (ColorStateList) resource.getColorStateList(R.color.text_hint); 
			user_info_sex_input.setTextColor(csl);
		}
		user_info_truename_input.setText((MyApplication.getInstances().getTruename()!="")?(MyApplication.getInstances().getTruename()):""); 
		user_info_address_input.setText((MyApplication.getInstances().getArea()!="")?(MyApplication.getInstances().getArea()):""); 
	...
	public void onClick(View v) {
		...
		case R.id.userinfo_edit_back:
			...
			UserGroupTab.getInstance().switchActivity("User", bak_My_intent, -1, -1);
			break;
	...
	private void submitData() {
		...
		if (!RegularUtil.checkTrueName(this, truename)) {
			...
			server_api_userinfo(truename, sex,address,MyApplication.getInstances().getUserName());        //接口请求
		}
		
		
	}

	private void server_api_userinfo(String truename, int sex,
            ...
            public void handleMessage(Message msg) {  
                if(msg.what==0x123){  
                	...
                	if(resmsg.get("code").equals("succeed")){
                		...
                		RegularUtil.alert_msg(UserInfo.this, "修改失败，"+resmsg.get("msg")); 
                	} 
        ...
        String params="mobile="+username+"&truename="+truename+"&area="+address+"&sex="+sex;
		...
		pDialog.show();
        new Thread(new AccessNetwork("POST", WebApiUrl.MEMBERINFO, params, h)).start();   
